...
def display_results(title: str, data, headers: list = None):
    """
    Display results in a formatted table.
    
    Args:
        title: Title of the results
        data: Iterable of dicts, list-like, or a Pandas DataFrame
        headers: Optional list of headers
    """
    print(f"\n{title}")
    print("-" * len(title))
    
    # Handle None or empty structures safely (including Pandas DataFrame)
    try:
        import pandas as pd  # type: ignore
    except Exception:
        pd = None  # pandas not strictly required for type checks
    ...
    # Fallback: attempt to tabulate any other iterable/records
